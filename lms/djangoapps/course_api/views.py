...
@view_auth_classes()
class CourseMentorsView(DeveloperErrorViewMixin, APIView):
    """
    Returns a list of mentors (instructors and staff) for a specific course.
    """
    def get(self, request, course_key_string):
        ...
        try:
            ...
            raise NotFound("Invalid course key.")
        ...
        for role_obj in roles:
            user = role_obj.user
            if user.id in mentors or not getattr(user, 'is_active', True):
                continue
        ...
        return Response({"mentors": list(mentors.values())})
